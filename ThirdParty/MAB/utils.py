...
try:
    _benign_dir = str(config['DATASET']['benign_content_folder'])
    list_benign_section_file = os.listdir(_benign_dir) if os.path.isdir(_benign_dir) else []
except Exception:
    list_benign_section_file = []
...
class Utils:
    ...
    def get_random_content():
        if not list_benign_section_file:
            # Fallback when benign_content_folder is empty/missing: use random bytes
            size = 512
            # content = os.urandom(size)
            content = random.getrandbits(size * 8).to_bytes(size, 'big')
            return 'default', size, content
        filename = random.choice(list_benign_section_file)
        try:
            name = filename.split('|')[1]
            size = int(filename.split('|')[2])
        except IndexError:  # filename does not contain |
            name = filename[:8]
            size = None
        try:
            file_path = str(config['DATASET']['benign_content_folder']) + filename
            with open(file_path, 'rb') as fp:
                content = fp.read()
                size = size or len(content)
                # Safety check: ensure content is not empty
                if len(content) == 0:
                    logger_rew.warning('Warning: benign content file %s is empty, using random bytes' % filename)
                    size = 512
                    content = random.getrandbits(size * 8).to_bytes(size, 'big')
                    name = 'default'
                return name, size, content
        except Exception as e:
            logger_rew.warning('Warning: failed to read benign content file %s: %s, using random bytes' % (filename, str(e)))
            size = 512
            content = random.getrandbits(size * 8).to_bytes(size, 'big')
            name = 'default'
            return name, size, content
...
logger_min.addHandler(logging.StreamHandler(sys.stdout))
