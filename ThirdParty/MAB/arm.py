...
class ArmSP(Arm):
    ...
    def transfer(self, input_path, output_folder=rewriter_output_folder, verbose=True):
        ...
        # Safety check: ensure content is not empty to prevent infinite loop
        if len(self.content) == 0:
            if verbose == True:
                logger_rew.warning('Warning: content is empty (len=0), generating new random content')
            _, _, self.content = Utils.get_random_content()
            if len(self.content) == 0:
                if verbose == True:
                    logger_rew.error('Error: cannot get valid content after retry, return original sample')
                os.system('cp -p %s %s' %(input_path, output_path))
                return output_path

        append_content = self.content
        if len(append_content) != 1:            # if it's SP1, do not need to extend content
            # Safety check: prevent infinite loop if content is empty
            if len(self.content) == 0:
                if verbose == True:
                    logger_rew.error('Error: content is empty, cannot extend, return original sample')
                os.system('cp -p %s %s' %(input_path, output_path))
                return output_path
            # Add max iterations to prevent infinite loop or very long execution
            # Calculate reasonable max iterations: (available_size / content_size) + safety margin
            if available_size > 0 and len(self.content) > 0:
                max_iterations = (available_size // len(self.content)) + 100
            else:
                max_iterations = 1
            iteration = 0
            while available_size > len(append_content) and iteration < max_iterations:   # extend content
                append_content += self.content
                iteration += 1
            if iteration >= max_iterations:
                if verbose == True:
                    logger_rew.warning('Warning: reached max iterations (%d) in content extension, available_size=%d, content_size=%d' % 
                                      (max_iterations, available_size, len(self.content)))
